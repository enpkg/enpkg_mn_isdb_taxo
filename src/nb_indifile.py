...
from reweighting_functions import chemical_reponderator, taxonomical_reponderator
...
with open("configs/user/user.yaml", "r", encoding="utf8") as file:
...
top_N_chemical_consistency = params_list["reweighting_params"][
...
for sample_dir in samples_dir[:]:
    ...
    if os.path.isfile(
        os.path.join(
            ...
            sample_dir,
            ionization_mode,
            sample_dir + "_features_ms2_" + ionization_mode + ".mgf",
        )
    ):
        pass
    else:
    ...
    if os.path.isfile(
        os.path.join(
            ...
            sample_dir,
            ionization_mode,
            sample_dir + "_features_quant_" + ionization_mode + ".csv",
        )
    ):
        pass
    else:
    ...
    if recompute == False:
        if os.path.isfile(
            os.path.join(
                repository_path, sample_dir, ionization_mode, "isdb/config.yaml"
            )
        ):
            print(
                sample_dir
...
for sample_dir in samples_dir:
    ...
    isdb_results_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/isdb/{sample_dir}_isdb_{ionization_mode}.tsv"
    )
    mn_ci_ouput_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/molecular_network/{sample_dir}_mn_metadata_{ionization_mode}.tsv"
    )
    repond_table_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/isdb/{sample_dir}_isdb_reweighted_{ionization_mode}.tsv"
    )
    repond_table_flat_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/isdb/{sample_dir}_isdb_reweighted_flat_{ionization_mode}.tsv"
    )
    mn_graphml_ouput_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/molecular_network/{sample_dir}_mn_{ionization_mode}.graphml"
    )
    treemap_chemo_counted_results_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/isdb/{sample_dir}_treemap_chemo_counted_{ionization_mode}.html"
    )
    treemap_chemo_intensity_results_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/isdb/{sample_dir}_treemap_chemo_intensity_{ionization_mode}.html"
    )
    isdb_config_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/isdb/config.yaml"
    )
    mn_config_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/molecular_network/config.yaml"
    )
    isdb_folder_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/isdb/"
    )
    mn_folder_path = os.path.normpath(
        f"{repository_path}/{sample_dir}/{ionization_mode}/molecular_network/"
    )
